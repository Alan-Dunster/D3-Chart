d3.csv('av2016.csv').then(data => draw(data))
...
function draw(data) {
  ...
  y.domain([-200, d3.max(data, function(d) { return d.percent; })]);


  previewX.domain(d3.extent(data, d => d.date));
  previewY.domain([-200, d3.max(data, d => d.percent)]);
 ...
 const nestBytypeId = d3.nest()
  ...
  nestByDate.forEach(dateItem => {
    ...
    dateItem.values.forEach(item => {
      percentsByDate[dateItem.key][item.typeId] = item.percent;
    });
  });
  ...
  function voronoiMouseover(d) {
    ...
    legendsValues.text(dataItem => {
      ...
      return value ? value + '%' : 'N.A.'; // Changed Н/Д from **return value ? value + '%': 'Н/Д';** to N.A. as Н/Д is Russian.
    });
    ...
    linesContainer.selectAll('path')
      .attr('d', typeId => {
        ...
        //.curve(d3.curveBasis)
          ...
          .x(d => rescaledX(d.date))
          .y(d => rescaledY(d.percent))(types[typeId].data);
      });
